...
export const updateProduct = async (accessToken, user, id, axiosJWT, dispatch) => {
    ...
    try {
        const res = await axiosJWT.put(`https://laptopstore-be.onrender.com/v1/user/update-product/` + id, {
            headers: { token: `Bearer ${accessToken}` },
            body: user,
        });
        alert('Cập nhật thông tin thành công');
...
export const updateOrder = async (orderId, orders, axiosJWT, dispatch) => {
    ...
    try {
        const res = await axiosJWT.put(`https://laptopstore-be.onrender.com/v1/user/update-order/` + orderId, {
            body: orders,
        });
        alert('Cập nhật thông tin thành công');
